...
public class EmployeeService {
    ...
    public ResponseEntity<List<Employee>> getEmployees() {
        ...
        try {
            ...
            log.warn("Too many request exception, building mock data {} ",tooManyRequests.getMessage());
        ...
        log.info("Fetched {} employees successfully",responseEntity.getBody().getData().size());
    ...
    public ResponseEntity<List<Employee>> getEmployeesByName(String searchString) {
        ...
        try {
            ...
            log.info("Fetched {} employees who's name contains {}", employeeList.size(), searchString);
    ...
    public ResponseEntity<Employee>  getEmployeeById(String id) {
        ...
        try {
            ...
            List<Employee> searchedEmployees = employeeResponse
                    .getData()
                    .stream()
                    .filter(employee -> employee.getId().toString().equals(id))
                    .collect(Collectors.toList());
            if(searchedEmployees.size() == 1) {
                log.info("Fetched employee with ID {} successfully",id);

                return ResponseEntity.ok(searchedEmployees.get(0));
            }
            log.error("Found two employees with same ID {}", id);
            return ResponseEntity.internalServerError().build();
        } catch (Exception exception) {
    ...
    public ResponseEntity<Integer> getHighestSalaryOfEmployees() {
        ...
        try {
            ...
            if (!employeeList.isEmpty()) {
                Optional<Employee> maxEmployee = employeeList
                        .stream()
                        .max(Comparator.comparingInt(EmployeeUtils::parseSalary));
                max = parseSalary(maxEmployee.get());
            }
            log.info("Fetched highest salary of employee successfully");
        ...
        try {
            ...
            if (!employeeList.isEmpty()) {
                employeeNames = employeeList
                        ...
                        .sorted(
                                Comparator
                                        .comparingInt(employee -> parseSalary((Employee) employee))
                                        .reversed()
                        )
                        .limit(10)
            ...
            log.info("Fetched top {} highest salary employee names", employeeNames.size());
